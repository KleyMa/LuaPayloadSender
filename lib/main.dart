...
class _SenderScreenState extends State<SenderScreen> {
  ...
  void _initForegroundTask() {
    FlutterForegroundTask.init(
      androidNotificationOptions: AndroidNotificationOptions(
        ...
        channelDescription: 'Manteniendo conexión activa con la consola',
  ...
  Future<void> _stopPayload() async {
    ...
    _socket?.destroy();
    _socket = null;
    await WakelockPlus.disable(); // ← libera wakelock
  ...
  Future<void> _sendPayload() async {
    ...
    try {
      ...
      await WakelockPlus.enable(); // ← mantiene CPU activo
      _log('Conectando a $ip:$port …', LogType.info);
      _socket = await Socket.connect(ip, port,
          timeout: const Duration(seconds: 10));
      _log('✓ Conexión establecida', LogType.success);

      // Iniciar foreground service para mantener conexión con pantalla apagada
      await _startForegroundService();

      // Enviar: [size uint64 LE] + [bytes del archivo]
      ...
      if (e.toString().contains('cancelled')) {
        // detenido manualmente, no mostrar error
      } else {
  ...
  Future<void> _processIncomingData(Socket socket) async {
    ...
    socket.listen(
      ...
      onError: (e) {
        _log('Error en stream: $e', LogType.error);
        if (!(_completer?.isCompleted ?? true)) _completer!.complete();
      },
      cancelOnError: true,
    );
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildFileCard(),
                  const SizedBox(height: 16),
                  _buildButtons(),          // ← botón enviar + detener
                  const SizedBox(height: 16),
                  _buildLogCard(),
                ],
              ),
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: const Icon(Icons.send_rounded, color: Color(0xFF00B4D8), size: 24),
        ),
        const SizedBox(width: 12),
        const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Lua Payload Sender',
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            color: _isSending ? const Color(0xFFFFB703) : const Color(0xFF2DC653),
            boxShadow: [BoxShadow(
              color: (_isSending
                  ? const Color(0xFFFFB703)
                  : const Color(0xFF2DC653)).withOpacity(0.6),
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(
                ...
                children: [
                  Text(
                    _selectedFileName ?? 'Toca para seleccionar archivo .lua',
                    style: TextStyle(
                      color: _selectedFileName != null
